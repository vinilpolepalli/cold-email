-- Sloan storage schema. Run once in the Supabase SQL editor (or via the
...
create table if not exists public.sloan_kv (
  key text primary key,
  value jsonb not null,
  updated_at timestamptz not null default now()
);

alter table public.sloan_kv enable row level security;
